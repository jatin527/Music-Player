...
statusbar = ttk.Label(root,text="Please select a song to play",background='cyan',font=('Comic Sans MS',11))
statusbar.pack()
statusbar1 = ttk.Label(root,text="Total Time : --:--",background='cyan',font=('Comic Sans MS',11))
statusbar1.pack()
statusbar2 = ttk.Label(root,text="Current Time : --:--",background='cyan',font=('Comic Sans MS',11))
statusbar2.pack()
...
def browse_song():
    try:
        global file_location,filename,played,play_song
        played=1
...
def play_music():
    ...
    if played==0 and len(playlist)==0:
        ...
        try:
            ...
            tkinter.messagebox.showerror("Select audio", "Select audio file from above toolbar. Click on file then open" )
    else:
        paused=0
        mixer.music.unpause()
...
scale = ttk.Scale(bframe, from_=0, to=125, orient=HORIZONTAL, command=set_vol)
scale.set(90)
...
def close_window():
    tkinter.messagebox.showinfo("Confirm Exit", "Are you really want to exit")
    root.destroy()
